  ...
  function render(){
    ...
    arr.forEach((ev, i)=>{
      ...
      tr.innerHTML=`<td>${ev.ref||'—'}</td><td>${(ev.loc||'—').toUpperCase()}</td>
        <td>${(ev.bruta||0).toFixed(1)} %</td><td>${(ev.neta||0).toFixed(1)} %</td>
        <td>${new Intl.NumberFormat('es-ES',{style:'currency',currency:'EUR',maximumFractionDigits:0}).format(ev.flujo||0)}</td>
        <td>${cli}</td>
        <td><button class="btn ghost">Cargar en Evaluar</button></td>`;
